#!/bin/bash
###############################################################################
# WordPress Comprehensive Site Diagnostic with Content & Link Validation
# Version: 1.3.0
# Created: 2025-11-08
# Updated: 2025-11-08 (Added Layer 10: Content & Link Validation)
# Protocol: wordpress_site_diagnostic_protocol.md
#
# Description:
#   Performs 10-layer diagnostic of WordPress site including WCAG 2.1 AA
#   accessibility checking, JavaScript runtime analysis, and content validation
#
# Changes from v1.2.0:
#   + Added LAYER 10: Content & Link Validation
#   + Expected pages existence check (core pages + all neighborhoods)
#   + Broken internal link detection
#   + Empty/Loading content detection
#   + Menu link validation
#   + Shortcode awareness (accepts dynamic content)
#
...
#
# Usage:
#   bash wordpress_comprehensive_diagnostic_v1.3.0.sh [site_url] [wp_path]
#
# Examples:
#   bash wordpress_comprehensive_diagnostic_v1.3.0.sh http://rundaverun-local.local
#   bash wordpress_comprehensive_diagnostic_v1.3.0.sh https://rundaverun.org /home/dave/html
#
# Exit Codes:
#   0 = All checks passed
...
###############################################################################
# LAYER 10: Content & Link Validation
###############################################################################
log_header "LAYER 10: Content & Link Validation"

# 10.1 - Expected Pages Existence Check
log_info "10.1 - Expected Pages Existence Check"

MISSING_PAGES=0

# Core pages to check
CORE_PAGES="home about-dave our-plan policies get-involved voter-education contact budget newsletter volunteer-registration volunteer-login volunteer-dashboard neighborhoods privacy-policy"

for page in $CORE_PAGES; do
    COUNT=$(wp post list --post_type=page --name="$page" --post_status=publish --format=count 2>/dev/null || echo "0")
    if [ "$COUNT" = "0" ]; then
        log_error "Missing core page: $page"
        ISSUES_FOUND=$((ISSUES_FOUND + 1))
        MISSING_PAGES=$((MISSING_PAGES + 1))
    fi
done

# All 42 neighborhood ZIP codes
EXPECTED_ZIPS="40023 40025 40041 40047 40059 40067 40109 40118 40177 40202 40203 40204 40205 40206 40207 40208 40209 40210 40211 40212 40213 40214 40215 40216 40217 40218 40219 40220 40221 40222 40223 40225 40228 40229 40241 40242 40243 40245 40258 40272 40291 40299"

for zip in $EXPECTED_ZIPS; do
    COUNT=$(wp post list --post_type=page --name="neighborhood-$zip" --post_status=publish --format=count 2>/dev/null || echo "0")
    if [ "$COUNT" != "1" ]; then
        log_error "Missing neighborhood page for ZIP: $zip (neighborhood-$zip)"
        ISSUES_FOUND=$((ISSUES_FOUND + 1))
        MISSING_PAGES=$((MISSING_PAGES + 1))
    fi
done
...
if [ "$MISSING_PAGES" -eq 0 ]; then
    log_success "All expected pages exist (13 core + 42 neighborhoods)"
else
    log_error "Total missing pages: $MISSING_PAGES"
fi

# 10.2 - Broken Internal Links
log_info "10.2 - Broken Internal Links"

BROKEN_LINKS=0

# Get all published posts and pages
ALL_POST_IDS=$(wp post list --post_type=any --post_status=publish --format=ids 2>/dev/null || echo "")

if [ -n "$ALL_POST_IDS" ]; then
    # Extract internal links from content
    INTERNAL_LINKS=$(echo "$ALL_POST_IDS" | xargs -n1 wp post get --field=post_content 2>/dev/null | grep -oP 'href="(/[^"]*)"' 2>/dev/null | sort -u || echo "")

    if [ -n "$INTERNAL_LINKS" ]; then
        TOTAL_LINKS=$(echo "$INTERNAL_LINKS" | wc -l)
        log_info "Found $TOTAL_LINKS unique internal links to validate"

        # Sample first 50 links to avoid timeout
        LINKS_TO_CHECK=$(echo "$INTERNAL_LINKS" | head -50)

        while IFS= read -r link_line; do
            if [ -n "$link_line" ]; then
                # Extract path from href="/path"
                LINK_PATH=$(echo "$link_line" | sed 's/href="//; s/"$//')

                # Extract slug from path (last segment)
                SLUG=$(echo "$LINK_PATH" | sed 's|/$||; s|.*/||')
                ...
                if [ -n "$SLUG" ]; then
                    POST_COUNT=$(wp post list --post_type=any --name="$SLUG" --post_status=publish --format=count 2>/dev/null || echo "0")
                    if [ "$POST_COUNT" = "0" ]; then
                        log_warning "Broken internal link: $LINK_PATH (no post/page with slug '$SLUG')"
                        WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
        ...
        if [ "$BROKEN_LINKS" -eq 0 ]; then
            log_success "No broken internal links detected (checked 50 sample links)"
        else
            log_warning "Found $BROKEN_LINKS broken internal links"
        fi
    else
        log_info "No internal links found to validate"
    fi
else
    log_warning "Could not retrieve post IDs for link checking"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
fi

# 10.3 - External Links (Optional - Skip for performance)
log_info "10.3 - External Links Check (Skipped)"
log_info "External link validation skipped for performance. Run pre_deployment_validator_v1.0.0.sh for full external link checking."

# 10.4 - Empty/Loading Content Check
log_info "10.4 - Empty/Loading Content Check"

EMPTY_PAGES=0
LOADING_PAGES=0

# Check all published pages for empty or "Loading..." content
PAGE_IDS=$(wp post list --post_type=page --post_status=publish --format=ids 2>/dev/null || echo "")
...
if [ -n "$PAGE_IDS" ]; then
    for page_id in $PAGE_IDS; do
        ...
        # Check for truly empty pages (less than 10 characters)
        if [ "$CONTENT_LENGTH" -lt 10 ]; then
            PAGE_TITLE=$(wp post get "$page_id" --field=post_title 2>/dev/null || echo "Unknown")
            log_error "Empty page detected: ID $page_id - $PAGE_TITLE"
            ISSUES_FOUND=$((ISSUES_FOUND + 1))
            EMPTY_PAGES=$((EMPTY_PAGES + 1))
        fi

        # Check for pages with shortcodes (acceptable - they render dynamically)
        if echo "$CONTENT" | grep -q "\[.*\]" 2>/dev/null; then
            # Shortcode detected - this is fine, skip to next
            continue
        fi

        # Check for literal "Loading..." text in content (bad - indicates broken shortcode)
        if echo "$CONTENT" | grep -qi "loading\.\.\." 2>/dev/null; then
            PAGE_TITLE=$(wp post get "$page_id" --field=post_title 2>/dev/null || echo "Unknown")
            log_warning "Page may have broken dynamic content: ID $page_id - $PAGE_TITLE"
            WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
            LOADING_PAGES=$((LOADING_PAGES + 1))
        fi
    done

    if [ "$EMPTY_PAGES" -eq 0 ] && [ "$LOADING_PAGES" -eq 0 ]; then
        log_success "No empty or broken pages detected"
    else
        if [ "$EMPTY_PAGES" -gt 0 ]; then
            log_error "Found $EMPTY_PAGES empty pages"
        fi
        if [ "$LOADING_PAGES" -gt 0 ]; then
            log_warning "Found $LOADING_PAGES pages with 'Loading...' text (may indicate broken shortcodes)"
        fi
    fi
else
    log_warning "Could not retrieve page IDs for content checking"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
fi

# 10.5 - Menu Link Validation
log_info "10.5 - Menu Link Validation"

BROKEN_MENU_LINKS=0

# Get all menus
MENUS=$(wp menu list --format=ids 2>/dev/null || echo "")

if [ -n "$MENUS" ]; then
    for menu_id in $MENUS; do
        MENU_NAME=$(wp menu list --format=csv 2>/dev/null | grep "^$menu_id," | cut -d, -f2 || echo "Unknown")

        # Get menu items
        MENU_ITEMS=$(wp menu item list "$menu_id" --format=ids 2>/dev/null || echo "")

        if [ -n "$MENU_ITEMS" ]; then
            for item_id in $MENU_ITEMS; do
                # Get the object_id (the post/page ID this menu item links to)
                OBJECT_ID=$(wp db query "SELECT object_id FROM wp_postmeta WHERE post_id=$item_id AND meta_key='_menu_item_object_id'" --skip-column-names 2>/dev/null || echo "")

                if [ -n "$OBJECT_ID" ] && [ "$OBJECT_ID" != "0" ]; then
                    # Check if the linked post/page exists and is published
                    POST_STATUS=$(wp post get "$OBJECT_ID" --field=post_status 2>/dev/null || echo "not_found")

                    if [ "$POST_STATUS" != "publish" ]; then
                        ITEM_TITLE=$(wp db query "SELECT post_title FROM wp_posts WHERE ID=$item_id" --skip-column-names 2>/dev/null || echo "Unknown")
                        log_warning "Menu item '$ITEM_TITLE' in menu '$MENU_NAME' links to unpublished/missing post ID $OBJECT_ID"
                        WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
                        BROKEN_MENU_LINKS=$((BROKEN_MENU_LINKS + 1))
                    fi
                fi
            done
        fi
    ...
    if [ "$BROKEN_MENU_LINKS" -eq 0 ]; then
        log_success "All menu links are valid"
    else
        log_warning "Found $BROKEN_MENU_LINKS broken menu links"
    fi
else
    log_info "No menus found or could not retrieve menu data"
fi
