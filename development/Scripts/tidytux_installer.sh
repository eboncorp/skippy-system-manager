...
install_scripts() {
    ...
    if [ -f "tidytux-tailored.sh" ]; then
        cp "tidytux-tailored.sh" "$INSTALL_DIR/tidytux"
        chmod +x "$INSTALL_DIR/tidytux"
        success "Installed tailored script as 'tidytux'"
    else
        warning "tidytux-tailored.sh not found in current directory"
    fi
    
    # Copy the GUI script
    if [ -f "tidytux-gui.sh" ]; then
        cp "tidytux-gui.sh" "$INSTALL_DIR/tidytux-gui"
        chmod +x "$INSTALL_DIR/tidytux-gui"
        success "Installed GUI script as 'tidytux-gui'"
    else
        warning "tidytux-gui.sh not found in current directory"
    fi
}
