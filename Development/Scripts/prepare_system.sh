#!/bin/bash
# Prepare system for home server installation
...
echo -e "${BLUE}Preparing system for Home Server installation${NC}"
echo -e "${YELLOW}This script will install required dependencies${NC}"
echo
...
if ! command -v pip3 &> /dev/null; then
    NEEDS_INSTALL="$NEEDS_INSTALL python3-pip"
fi

if ! python3 -c "import venv" &> /dev/null 2>&1; then
    NEEDS_INSTALL="$NEEDS_INSTALL python3-venv"
fi

if ! python3 -c "import distutils" &> /dev/null 2>&1; then
    NEEDS_INSTALL="$NEEDS_INSTALL python3-distutils"
fi
...
if [ -n "$NEEDS_INSTALL" ]; then
    ...
    echo
    echo "Please run:"
    echo -e "${GREEN}sudo apt update && sudo apt install -y$NEEDS_INSTALL${NC}"
    echo
    echo "Then run the installer again:"
    echo "./home_server_installer.sh"
else
    echo -e "${GREEN}✓ All dependencies are installed!${NC}"
    echo "You can now run: ./home_server_installer.sh"
fi

# Also create a non-sudo installer that skips pip installation
cat > /home/dave/Skippy/home_server_installer_nosudo.sh << 'EOF'
#!/bin/bash
# Home Server Master Installer (No sudo version)
# For systems where dependencies are already installed
...
print_header() {
    ...
    echo -e "${BLUE}   (No sudo version)${NC}"
    echo -e "${BLUE}================================================${NC}"
    echo
}
...
install_dependencies() {
    ...
    if ! python3 -m venv "$INSTALL_DIR/venv"; then
        ...
        print_error "Please install python3-venv: sudo apt install python3-venv"
        exit 1
    fi
...
copy_files() {
    ...
    if [ -d "$SCRIPT_DIR/Downloads" ]; then
        ...
        find "$SCRIPT_DIR/Downloads" -name "*.sh" -exec cp {} "$INSTALL_DIR/Downloads/" \; 2>/dev/null || true
    fi
...
create_default_config() {
    ...
    cat > "$CONFIG_DIR/server.yaml" << CONFIG_EOF
...
setup_systemd_service() {
    ...
    cat > "$SYSTEMD_USER_DIR/home-server.service" << SERVICE_EOF
[Unit]
...
chmod +x /home/dave/Skippy/home_server_installer_nosudo.sh

echo -e "\n${GREEN}Created alternative installer: home_server_installer_nosudo.sh${NC}"