...
class TransactionHistory:
    ...
    def get_unrealized_gains(self, current_prices: Dict[str, float]) -> Dict[str, dict]:
        ...
        for asset, lots in self.tax_lots.items():
            remaining_lots = [lot for lot in lots if lot.remaining_amount > 0]

            if not remaining_lots:
                continue

            total_amount = sum(lot.remaining_amount for lot in remaining_lots)
            total_cost_basis = sum(lot.remaining_amount * lot.cost_basis_per_unit for lot in remaining_lots)
